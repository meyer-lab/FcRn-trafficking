...
stan_plot(fit, pars = c("Q", "Vp", "Qu", "Vin"))
```

XX

```{r, fig.height=2, fig.width=5}
stan_plot(fit, pars = c("actual_sortF_wt", "actual_sortF_dhs", "actual_sortF_ls", "sortF_yte",
...
```{r}
pairs(fit, pars = c("actual_sortF_wt", "actual_sortF_dhs", "actual_sortF_ls", "sortF_yte",
                    "actual_release_ls", "releaseF_yte"),
      lower.panel = sum,
      gap = 0.25)
```
...
Lastly, I've plotted the predicted half-life for a particular sorting and release fraction below, taking into account the uncertainty of the previous fitting. The plot shows the prediction's 80% prediction intervals.
...
ggplot2::ggplot(output, ggplot2::aes(x = sortF,
                                     color = as.factor(releaseF),
                                     group = releaseF)) +
  ggplot2::geom_ribbon(ggplot2::aes(ymin = lwr,
                                    ymax = upr,
                                    fill = as.factor(releaseF)),
                       linetype = "blank",
                       alpha = 0.6) +
  ggplot2::xlim(0, 1) +
  ggplot2::theme_bw() +
